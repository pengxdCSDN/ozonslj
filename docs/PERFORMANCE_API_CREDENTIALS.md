...
4. 点击“获取 Token 并测试连接”；后端调用 API 专用地址 `https://api-performance.ozon.ru/api/client/token`，保存短期 access token 并返回脱敏状态。`performance.ozon.ru` 是网页入口，不得作为后端 API 地址。
...
Token 请求的重定向只允许继续访问 `api-performance.ozon.ru` 的 HTTPS 地址，最多跟随 3 次；HTTP、网页域名、其他域名、缺少 Location 或循环重定向会被安全拦截，避免凭据被转发到非受信目标。
