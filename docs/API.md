...
- 当前已实现接口具备服务端请求状态到 PostgreSQL RLS 的上下文桥接，并由登录会话装配 `TenantContext`；缺少认证上下文时返回 `401`。真实云端开放前仍需完成隔离 PostgreSQL/RLS 集成验收，不能仅依赖路径中的 `workspace_id`，也不能信任客户端自报租户请求头。
...
当前接口通过 PostgreSQL 适配器读取商品事实，并受请求组织上下文与 RLS 约束；业务切片和迁移已完成。隔离 PostgreSQL/RLS 的真实部署验收仍需在目标环境执行，不能用本地 Stub 测试替代。
