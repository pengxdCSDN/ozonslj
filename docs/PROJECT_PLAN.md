...
知识型混合 RAG 的需求与架构已经定档，核心本地闭环和开发云安全恢复闭环已完成；专项基线见 [RAG 需求](./RAG_REQUIREMENTS.md)、[RAG 技术架构](./RAG_ARCHITECTURE.md) 和 [RAG 详细实施计划](./RAG_IMPLEMENTATION_PLAN.md)。当前剩余工作是固定评测集实际质量运行、真实供应商持续验收和 Seller/Performance 外部授权后的真实只读接入。
...
| V5.6 | 知识型混合 RAG：接入、切片、Chroma/关键词检索、意图路由、精排、引用、拒答、评测和模型降级 | 已开发（本地与开发云安全恢复闭环） | API/页面/测试、固定 400 例、确认持久化和云端健康恢复通过；240 例实际质量运行及真实供应商授权验收仍待完成 |
