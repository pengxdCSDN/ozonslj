...
跨页面事件驱动和定时自动化开始前，必须先完成统一编排基础，详细约束见 [自动化优先设计](./AUTOMATION_FIRST_DESIGN.md)：

1. 定义外部事实、领域事实、计算结果、建议和展示刷新五类事件，并采用下游白名单触发。
2. 任务保存 `run_id`、根任务、父任务、触发来源和数据版本，形成可追踪的单向触发链。
3. 建立业务幂等键、唯一约束、最大链路深度、有限重试、指数退避和熔断转人工机制。
4. 禁止页面加载触发同步、计算结果修改规则、建议直接执行以及通知失败触发同类型通知。
5. 通过重复消息、Worker 重启、部分失败、认证失败和循环事件等测试后，才能开放跨页面自动联动。

自动化功能按照“编排基础 → P0 数据可信闭环 → P1 定时/事件驱动 → P2 回读对账与持续优化”实施；页面功能不得绕过统一编排层自行互相触发。

当前进度：编排基础的领域规则、防循环回归测试、同步任务运行上下文字段、Worker 成功后的事实变化事件发布、事件白名单消费者、重复消息幂等、租约恢复、质量任务熔断和 P0 数据质量页面联动已完成。后续开发必须继续沿用事件白名单和单向触发链，优先扩展库存/订单/履约规则与领域计算结果回读，不得由展示刷新反向触发外部同步或写接口。
...
所有阶段统一遵循“一切尽可能自动化”的受控自动化原则。页面级自动化改进、优先级和验收指标见 [自动化优先设计](./AUTOMATION_FIRST_DESIGN.md)；阶段完成不仅要求页面和接口存在，还要尽量形成自动获取、校验、触发、回读和复盘闭环。
