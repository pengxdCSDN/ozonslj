...
- 正式评测索引采用按套件懒加载和按切片 ID 断点续传：quick/standard/full 只为当前冻结案例建立 Embedding，重试只补 Chroma 缺失的 `gold-*` 切片；完整策略、前后消耗和回滚约束见 [`RAG_TOKEN_USAGE_OPTIMIZATION`](../RAG_TOKEN_USAGE_OPTIMIZATION.md)。
