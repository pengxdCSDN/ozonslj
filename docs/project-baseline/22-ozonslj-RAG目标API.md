...
评测案例采用离线任务接口：`POST /v1/rag-evaluation-case-generation-jobs` 创建受控 AI 草稿生成任务，`GET /v1/rag-evaluation-cases` 查看脱敏草稿，`POST /v1/rag-evaluation-cases/{case_id}/confirm` 由有权限人员确认，`POST /v1/rag-evaluation-runs` 启动分层评测。生成模型不能调用确认接口；高风险案例确认要求 reviewer/admin。所有任务单并发分批执行并返回可暂停、续跑的任务 ID。
