...
所有环境只使用 PostgreSQL 作为业务数据库。Redis 不是业务数据库；SQLite、`DATABASE_PATH`、DPAPI 持久化和 SQLite 测试路径属于必须在 V5.1 清理的历史实现，不保留本地兼容运行模式。
...
### 15.1 PostgreSQL 备份与恢复（继承 V5，尚未开发）

仓库当前只有备份需求和架构说明，没有可运行的备份脚本、定时器、Compose 服务或恢复验证，因此不得把备份标记为已开发。实现时继承架构 V5 已定方案，并补齐需求 V5 的时间与保留规则：
