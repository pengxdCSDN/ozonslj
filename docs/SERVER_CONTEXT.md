...
- 稳定构建的完整门禁和发布记录模板见 `docs/ACR_STABLE_BUILD_FLOW.md`；基础镜像规则不得与应用开发分支绑定，否则一次代码提交会触发无关基础镜像队列。

当前仓库新增开发不得重复设计平行迁移账本、备份体系或凭据密钥方案。状态判断必须区分“代码/模板已存在”“服务器已安装”“实际执行已验证”；截至 2026-08-16，备份定时任务、日志轮转和一次隔离恢复演练均已完成验证。
