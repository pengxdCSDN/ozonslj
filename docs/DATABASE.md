...
Worker 使用 `FOR UPDATE SKIP LOCKED` 原子领取最早的排队任务，也可以接管租约已经过期的
运行任务。每次领取都会递增 `attempt_count`，该值同时作为心跳、成功和失败写入的栅栏
令牌；任务被新尝试接管后，旧 Worker 即使恢复也不能覆盖新尝试的状态。默认租约 60 秒、
每 20 秒续租，适配当前 2 核 2GB 单 Worker 节点，并允许后续安全增加 Worker 数量。
