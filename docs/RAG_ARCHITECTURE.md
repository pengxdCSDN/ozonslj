...
> 状态：架构已确认，PostgreSQL + Chroma 持久化运行时已接入；云端数据迁移、真实供应商和重建演练仍需部署验收。本文是知识型混合 RAG 的专项技术架构基线；具体模型与性能参数在 `shadow`/压测中版本化定档，不属于未确认架构。
...
生产 API/Worker 通过 `CHROMA_URL` 访问固定的 `ozonslj_knowledge` collection。来源、版本、切片正文和生命周期以 PostgreSQL 为准；只有质量门禁通过且版本发布成功的切片才 upsert 到 Chroma，撤回/软删除会按稳定 `chunk_id` 删除向量。API 重启后向量结果携带完整引用元数据，不依赖进程内存。
...
扫描 PDF 由 OCR 适配器输出页面 Markdown 和版面块后，可复用 `pdf_layout_blocks`；OCR 结果必须记录供应商/版本、页面置信度、资源预算和复核状态。不同业务的策略和参数必须版本化，切片预览通过后才进入嵌入阶段。
...
领域层只依赖上述解析产物，不依赖具体 OCR 类型。文本层缺失时调用受控 OCR 适配器；未配置、失败、空正文或质量不足的结果进入 `needs_review`，禁止继续发布。OCR 供应商凭据只由运行时 Secret 提供。
...
| 扫描 PDF | OCR 页面 Markdown/版面块后使用 `pdf_layout_blocks` 或 `pdf_paragraphs` | 先通过 OCR 质量门槛 | OCR 供应商、页码、置信度、布局警告、人工复核状态 |
