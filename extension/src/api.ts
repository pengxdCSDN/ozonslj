...
export interface AdvertisingBudgetAnalysis { budget_minor: number; spend_minor: number; days_elapsed: number; days_total: number; utilization_percent: number; projected_spend_minor: number; projected_utilization_percent: number; status: "healthy" | "warning" | "at_risk" | "exceeded"; recommendations: string[]; read_only: boolean; }
export function analyzeAdvertisingBudget(payload: Record<string, unknown>): Promise<AdvertisingBudgetAnalysis> { return requestJson("/v1/advertising/budget/analyze", { method: "POST", body: JSON.stringify(payload) }); }
...
export interface PerformanceCredentialStatus { credential_scope: string; client_id_present: boolean; client_secret_present: boolean; access_token_present: boolean; refresh_token_present: boolean; expires_at: string | null; isolated_from_seller: boolean; ready: boolean; }
