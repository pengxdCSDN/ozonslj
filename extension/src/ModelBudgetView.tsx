  ...
  return <div className="view-content budget-view">
    ...
    <section className="panel budget-panel">
      ...
      {rows.length ? rows.map((row) => { const key = row.provider.provider_id; const draft = getDraft(row); const budget = row.current; return <article className="operation-row" key={key}>
        ...
        <div className="budget-row-state"><em className={budget?.state === "exceeded" ? "danger" : budget?.state === "warning" ? "warning" : budget ? "success" : "pending"}>{budget?.state === "exceeded" ? "已阻断" : budget?.state === "warning" ? "接近上限" : budget ? "运行正常" : "待配置"}</em><>{budget ? <><small>今日 {budget.usage.daily_tokens.toLocaleString()} / {budget.policy.daily_token_limit.toLocaleString()} tokens</small><div className="budget-limit-details">{budget.metrics.map((metric) => <span className={`budget-limit-item ${metric.state}`} key={metric.key} title={`${metric.label}占比 ${(metric.ratio * 100).toFixed(1)}%`}>{metric.label} {metric.used.toLocaleString()} / {metric.limit.toLocaleString()}</span>)}</div></> : <small>保存策略后开始记录用量</small>}</></div>
