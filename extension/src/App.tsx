...
export function App() {
  ...
  return <main className="app-shell">
    <button className="text-button quality-shortcut" onClick={() => setView("quality")} type="button"><ShieldCheck size={14} /> 数据质量</button>
    ...
    {view === "performance-credentials" && active ? <PerformanceCredentialsView /> : null}
    ...
    <label className="workspace-switcher" title={operationFreshness ? `最近同步：${new Date(operationFreshness).toLocaleString("zh-CN")}` : "暂无同步时间"}><span className={`online-dot ${selectedWorkspace?.status ?? "pending"}`} /><span>当前店铺</span><select value={selectedWorkspaceId} onChange={(e) => changeWorkspace(e.target.value)}>{workspaces.map((item) => <option value={item.id} key={item.id}>{item.display_name}</option>)}</select><CaretDown size={12} /></label>
    <nav className="view-tabs"><button className={view === "overview" ? "active" : ""} onClick={() => setView("overview")}><ChartLineUp size={17} />总览</button><button className={view === "products" ? "active" : ""} onClick={() => setView("products")}><Package size={17} />商品{metrics.empty ? <span>{metrics.empty}</span> : null}</button><button className={view === "operations" ? "active" : ""} onClick={() => setView("operations")}><Truck size={17} />运营</button><button className={view === "tasks" ? "active" : ""} onClick={() => setView("tasks")}><ClipboardText size={17} />任务</button><button className={view === "accounts" ? "active" : ""} onClick={() => setView("accounts")}><Storefront size={17} />店铺</button></nav>
