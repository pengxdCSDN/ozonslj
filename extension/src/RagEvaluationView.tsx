...
export function RagEvaluationView() {
  ...
  return <div className="view-content">
    ...
    <section className="panel"><div className="section-heading"><div><span className="eyebrow">人工确认</span><h2>固定评测语料</h2></div><ClipboardText size={24} /></div>
      <div className="metric-grid"><div><small>案例总数</small><strong>{total}</strong></div><div><small>待确认</small><strong>{draftCount}</strong></div><div><small>已确认</small><strong>{confirmedCount}</strong></div></div>
      <div className="toolbar"><label>搜索案例<input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="案例 ID、问题、状态或安全标签" /></label></div>
      ...
      <div className="sync-actions"><button type="button" className="secondary-button" onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={loading || page <= 1}>上一页</button><span>第 {page} / {totalPages} 页</span><button type="button" className="secondary-button" onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={loading || page >= totalPages}>下一页</button></div>
    </section>
