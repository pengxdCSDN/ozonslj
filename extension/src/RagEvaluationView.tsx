...
export function RagEvaluationView() {
  ...
  return <div className="view-content rag-evaluation-view">
    ...
    <section className="panel rag-evaluation-panel"><div className="section-heading"><div><span className="eyebrow">人工确认</span><h2>固定评测语料</h2></div><ClipboardText size={24} /></div>
      ...
      <div className="sync-actions rag-pagination"><span>共 {total} 个案例 · 第 {page} / {totalPages} 页</span><div><button type="button" className="secondary-button" onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={loading || page <= 1}>上一页</button><button type="button" className="secondary-button" onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={loading || page >= totalPages}>下一页</button></div></div>
    </section>
