...
export function ProfitModelView({ workspaceId: _workspaceId }: { workspaceId: string }) {
  ...
  const saveReconciliation = async () => {
    ...
    try {
      await saveProfitReconciliation(_workspaceId, {
      ...
      setMessage("对账批次已保存；重复点击会复用同一幂等批次。 ");
    } catch (error) {
  ...
  return <div className="view-content profit-workbench">
    ...
    <div className="profit-layout">
      ...
      <button className="secondary-button" disabled={!reconciliation?.rows.length || busy !== "" || reconciliationSaved} onClick={() => void saveReconciliation()} type="button">{reconciliationSaved ? "对账已保存" : "保存对账批次"}</button></aside>
    </div>
