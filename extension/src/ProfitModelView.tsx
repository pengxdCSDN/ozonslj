...
export function ProfitModelView({ workspaceId: _workspaceId }: { workspaceId: string }) {
  ...
  const [persistedReconciliation, setPersistedReconciliation] = useState<ProfitReconciliationRecord[]>([]);
  ...
  const saveReconciliation = async () => {
    ...
    setBusy("save-reconcile");
    try {
      const saved = await saveProfitReconciliation(_workspaceId, {
        idempotency_key: `csv-${_workspaceId}-${new Date().toISOString().slice(0, 10)}-${reconciliation.rows.length}`,
        source: "operator_csv_preview",
        status: reconciliation.errors.length ? "partial" : "completed",
        records: reconciliation.rows.map((row) => ({
          order_id: row.order_id, sku_id: row.sku_id,
          estimated_profit_minor: row.estimated_profit_minor,
          actual_profit_minor: row.actual_profit_minor,
          variance_minor: row.variance_minor, side: "matched" as const, source: row.source,
        })),
      });
      const records = await listProfitReconciliationRecords(_workspaceId, saved.batch.id);
      setPersistedReconciliation(records);
      setReconciliationSaved(true);
      setMessage(`对账批次已保存并完成回读（${records.length} 条）；重复点击会复用同一幂等批次。 `);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "保存对账批次失败");
    } finally { setBusy(""); }
  };
  ...
  return <div className="view-content profit-workbench">
    ...
    <div className="profit-layout">
      ...
      <button className="secondary-button" disabled={!reconciliation?.rows.length || busy !== "" || reconciliationSaved} onClick={() => void saveReconciliation()} type="button">{reconciliationSaved ? "对账已保存" : "保存对账批次"}</button>{persistedReconciliation.length ? <div className="csv-preview reconciliation-readback" role="status"><strong>已回读 {persistedReconciliation.length} 条持久化明细</strong><small>批次来源：{persistedReconciliation[0].source} · 首条差异：{persistedReconciliation[0].variance_minor ?? "—"} 分</small></div> : null}</aside>
    </div>
